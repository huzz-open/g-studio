...
func recognize(scene_root: Node) -> Dictionary:
	...
	for child in scene_root.get_children():
		if _is_background_sprite(child, scene_root):
			...
			if sprite.texture:
				result["texture"] = sprite.texture.resource_path
...
func _get_img_center(scene_root: Node) -> Vector2:
	for child in scene_root.get_children():
		if child is Sprite2D and child.get_parent() == scene_root:
			var sprite = child as Sprite2D
			if sprite.texture:
...
func _extract_occlude_region(area: Area2D, img_center: Vector2) -> Dictionary:
	...
	for group in area.get_groups():
		region["groups"].append(str(group))

	for child in area.get_children():
		if child is CollisionPolygon2D:
			var cpoly = child as CollisionPolygon2D
			region["verts"] = _polygon_to_verts(cpoly.polygon, img_center)
			return region
		elif child is CollisionShape2D:
			var cs = child as CollisionShape2D
			if cs.shape is RectangleShape2D:
				region["rect"] = _shape_to_rect(cs, img_center)
				return region
...
func _extract_collision_region(node: Node, container_name: StringName, img_center: Vector2) -> Dictionary:
	...
	if node is CollisionPolygon2D:
		var cpoly = node as CollisionPolygon2D
		return {
			"name": rname,
			"type": 2,
			"verts": _polygon_to_verts(cpoly.polygon, img_center),
		}
		...
		if cs.shape is RectangleShape2D:
			return {
				"name": rname,
				"type": 2,
				"rect": _shape_to_rect(cs, img_center),
			}
	return {}
...
func _create_collision_node(parent: Node, region: Dictionary, owner: Node, img_center: Vector2) -> void:
	var rname = str(region.get("name", "Collision"))
...
func _update_occlude_node(area: Area2D, region: Dictionary, img_center: Vector2) -> void:
	...
	for group in area.get_groups():
		if not desired_groups.has(str(group)):
	...
	for child in area.get_children():
		if child is CollisionPolygon2D and region.has("verts"):
			var cpoly = child as CollisionPolygon2D
			cpoly.polygon = _verts_to_polygon(region.get("verts"), img_center)
			return
